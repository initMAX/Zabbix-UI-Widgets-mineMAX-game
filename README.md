<!-- *********************************************************************************************************************************** -->
<!-- *** HEADER ************************************************************************************************************************ -->
<!-- *********************************************************************************************************************************** -->
<div align="center">
    <a href="http://www.initmax.com"><img src="./.readme/logo/initMAX_banner.png" alt="initMAX Logo"></a>
    <h3>
        <span>
            Honesty, diligence and MAXimum knowledge of our products is our standard.
        </span>
    </h3>
    <h3>
        <a href="https://www.initmax.com/">
            <img alt="Static Badge" src="https://img.shields.io/badge/initMAX.com-%20?color=%231f65f4&logo=data:image/png;base64,">
        </a>&nbsp;
        <a href="https://www.linkedin.com/company/initmax/">
            <img alt="Static Badge" src="https://img.shields.io/badge/%20-%20?style=social&logo=linkedin">
        </a>&nbsp;
        <a href="https://www.youtube.com/@initmax1">
            <img alt="Static Badge" src="https://img.shields.io/badge/%20-web?style=social&logo=youtube">
        </a>&nbsp;
        <a href="https://www.facebook.com/initmax">
            <img alt="Static Badge" src="https://img.shields.io/badge/%20-%20?style=social&logo=facebook">
        </a>&nbsp;
        <a href="https://www.instagram.com/initmax/">
            <img alt="Static Badge" src="https://img.shields.io/badge/%20-%20?style=social&logo=instagram">
        </a>&nbsp;
        <a href="https://twitter.com/initmax">
            <img alt="Static Badge" src="https://img.shields.io/badge/%20-%20?style=social&logo=x">
        </a>&nbsp;
        <a href="https://github.com/initmax">
            <img alt="Static Badge" src="https://img.shields.io/badge/%20-%20?style=social&logo=github">
        </a>
    </h3>
    <h3>
        <a><img src="./.readme/logo/zabbix-premium-partner.png" alt="Zabbix premium partner" width="100"></a>&nbsp;&nbsp;&nbsp;
        <a><img src="./.readme/logo/zabbix-certified-trainer.png" alt="Zabbix certified trainer" width="100"></a>
    </h3>
</div>
<br>
<br>

---
---

<div align="center">
    <h1>
        mineMAX <!-- !!! change version !!! -->
    </h1>
    <h4>
        Classic Minesweeper game for Zabbix <!-- !!! change version !!! -->
    </h4>
    <br>
    <img alt="Static Badge" src="https://img.shields.io/badge/Required%20Zabbix%20version-7.0-red"> <!-- !!! change version !!! -->
    <img alt="Static Badge" src="https://img.shields.io/badge/Required%20php%20version-8.0-blue"> <!-- !!! change version !!! -->
    <h3>
        <a href="#description">Description</a> •<!-- !!! change list !!! -->
        <a href="#how-to-play">How to play</a> •
        <a href="#configuration">Configuration</a> •
        <a href="#installation">Installation</a>
    </h3>
    <br>
    <img src="./.readme/screen/mineMAX.png" width="1000"> <!-- !!! change version !!! -->
</div>
<br>
<br>

<!-- *********************************************************************************************************************************** -->
<!-- *** BODY ************************************************************************************************************************** -->
<!-- *********************************************************************************************************************************** -->
## Description
Experience the timeless challenge of Minesweeper directly within your Zabbix environment with mineMAX. This widget brings the classic game to your monitoring dashboard, offering a fun and engaging way to take short breaks. Identify all mines without triggering any, enhance your problem-solving skills, and enjoy a nostalgic gaming experience during your downtime.
<!-- *********************************************************************************************************************************** -->
<br>

## How to play
1. Click on a cell to reveal it.
2. Numbers indicate how many mines are in the adjacent cells.
3. Right-click to flag a suspected mine.
4. Clear all non-mine cells to win.
5. Avoid clicking on a mine, or the game ends!

Strategize, deduce, and enjoy this classic puzzle game right in your Zabbix dashboard.
<!-- *********************************************************************************************************************************** -->
<br>

## Configuration
<div align="center">
    <img src="./.readme/screen/mineMAX_conf.png" width="600"> 
</div><br>
To set up the mineMAX widget, navigate to the widget configuration panel. Here you can customize various aspects of the game:

- **Type**: Select "mineMAX" from the dropdown menu.
- **Name**: Give your widget a name (default is "default").
- **Refresh interval**: Set to "Default (No refresh)" for uninterrupted gameplay.
- **Player name**: Enter your desired player name (e.g., "initMAX").
- **Difficulty**: Choose between Beginner, Intermediate, or Expert levels.
- **Rows**: Set the number of rows for the game grid (e.g., 15).
- **Columns**: Set the number of columns for the game grid (e.g., 30).

Adjust these settings to tailor the game to your preference and skill level.
<!-- *********************************************************************************************************************************** -->
<br>

<!-- *********************************************************************************************************************************** -->
<!-- *** Installation ************************************************************************************************************************** -->
<!-- *********************************************************************************************************************************** -->
## Installation
- Connect to your Zabbix frontend server (perform on all frontend nodes)

- Navigate to the `ZBXUI/modules/` directory (`ZBXUI` is typically located at `/usr/share/zabbix/`)
    ```sh
    cd /usr/share/zabbix/modules/
    ```

- Clone repository on your server <!-- !!! repository !!! -->
    ```sh
    git clone https://git.initmax.cz/initMAX-Public/Zabbix-UI-Widgets-mineMAX-game.git
    ``` 

- Change the owner of directory to your web user with `chown`
  - default OS users
    - nginx
        ```sh
        chown nginx:nginx ./Zabbix-UI-Widgets-mineMAX-game
        ``` 
    - apache (RHEL)
        ```sh
        chown apache:apache ./Zabbix-UI-Widgets-mineMAX-game
        ``` 
    - apache (DEB)
        ```sh
        chown www-data:www-data ./Zabbix-UI-Widgets-mineMAX-game
        ``` 

- Go to Zabbix frontend menu -> Administration -> General -> Modules
<div align="center">
    <img src="./.readme/installation/menu.png" width="200">
</div>

- Use the Scan directory button on the top
<div align="center">
    <img src="./.readme/installation/scan_directory.png" width="1000">
</div>

- Enable the newly discovered module
<div align="center">
    <img src="./.readme/installation/enable.png" width="1000">
</div>

- Done
<div align="center">
    <img src="./.readme/installation/enable_done.png" width="1000">
</div>

- Use it and enjoy!

<!-- *********************************************************************************************************************************** -->
<!-- *** FOOTER ************************************************************************************************************************ -->
<!-- *********************************************************************************************************************************** -->
<br>
<br>

---
---
<div align="center">
    <h4>
        <a href="https://www.initmax.com/">
            <img alt="Static Badge" src="https://img.shields.io/badge/initMAX.com-%20?color=%231f65f4&logo=data:image/png;base64,">
        </a>
        <a href="tel:+420800244442">
            <img alt="Static Badge" src="https://img.shields.io/badge/+420%20800%20244%20442-%20?color=%231f65f4&logo=data:image/png;base64,">
        </a>
        <a href="mailto:info@initmax.com">
            <img alt="Static Badge" src="https://img.shields.io/badge/info@initmax.com-%20?color=%231f65f4&logo=data:image/png;base64,">
        </a>
        <br>
        <a href="https://www.linkedin.com/company/initmax/">
            <img alt="Static Badge" src="https://img.shields.io/badge/%20-%20?style=social&logo=linkedin">
        </a>&nbsp;
        <a href="https://www.youtube.com/@initmax1">
            <img alt="Static Badge" src="https://img.shields.io/badge/%20-web?style=social&logo=youtube">
        </a>&nbsp;
        <a href="https://www.facebook.com/initmax">
            <img alt="Static Badge" src="https://img.shields.io/badge/%20-%20?style=social&logo=facebook">
        </a>&nbsp;
        <a href="https://www.instagram.com/initmax/">
            <img alt="Static Badge" src="https://img.shields.io/badge/%20-%20?style=social&logo=instagram">
        </a>&nbsp;
        <a href="https://twitter.com/initmax">
            <img alt="Static Badge" src="https://img.shields.io/badge/%20-%20?style=social&logo=x">
        </a>&nbsp;
        <a href="https://github.com/initmax">
            <img alt="Static Badge" src="https://img.shields.io/badge/%20-%20?style=social&logo=github">
        </a>
        <br><br><br>
        <a>
            <img src="./.readme/logo/agplv3.png" width="100">
        </a>
    </h4>
</div>